...
void computeHash(const string& hashProgName)
{
  ...
  */
  const char* fileOpen = cmdLine.c_str();
  FILE* progOutput = popen(fileOpen, "r");
  if(!progOutput)
  {
    perror("popen");
    exit(-1);
...
void parentFunc(const string& hashProgName)
{
  ...
  if (write(parentToChildPipe[WRITE_END], filetoSend, sizeof(filetoSend)) < 0)
  {
    perror("write");
    exit(-1);
  }
  ...
  */
  ...
  fprintf(stderr, "Parent received hash from the child: %s\n\n",
  hashValue);
